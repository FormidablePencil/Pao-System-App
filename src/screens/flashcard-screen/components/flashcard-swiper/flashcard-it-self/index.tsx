...
import usePrimaryControlledColor, {
  WhereToColor,
  distinguishingTextColorFromRestOfText,
...
const FlashcardItSelf = ({ collection, index }: FlashcardsTypes) => {
  ...
  const flashcardItemDisplayedFront = useSelector(
    (state: any) => state.flashcardOptions.flashcardItemDisplayedFront
  );
  const editMode = useSelector(
    (state: RootReducerT) => state.fabProperties.config.editMode
  );
  const guessingFeatureOn = useSelector(
    (state: RootReducerT) => state.studyRandomMode.guessingFeatureOn
  );
  const [
    controlledInputs,
    setControlledInputs,
  ] = useState<ControlledInputsTypes>({
    data: [{ number: null, name: null, value: null }],
  });
  const [
    frontSideCurrentlyDisplayed,
    setFrontSideCurrentlyDisplayed,
  ] = useState(true);
  ...
  const sides = [
    {
      side: "front",
      interpolation: backInterpolation,
      opacity: backSideOpacity,
      symbol: true,
    },
    {
      side: "back",
      interpolation: frontInterpolation,
      opacity: frontSideOpacity,
      symbol: false,
    },
  ];
  ...
  const formatPaoItems = (key) => {
    if (
      ...
      if (
        typeof collection[key] === "number" &&
        collection[key].toString().length === 1
      )
    ...
    <>
      {sides.map((sidesDocument: any) => (
        <>
          ...
          {sidesDocument.side === "back" &&
            isRandomStudyMode &&
            guessingFeatureOn && <GuessingFeature index={index} />}
        </>
      ))}
    </>
  );
...
const PaoEmpty = ({ flashcardItemDisplayedFront, symbol }: any) => {
  const arrOfTrues = flashcardItemDisplayedFront.filter(
    (item) => Object.values(item)[0] === !symbol
  );
  if (arrOfTrues[3]) {
    return (
...
const AnimatedFlashcard = createAnimatableComponent(View);
