...
import NavigateToPaoTable, {
  NavigateToFlashcards,
} from "../components/ScreenHeaderComponents";
...
const StackNavigator = () => {
  ...
  useEffect(() => {
    dispatch({
      type: SET_PAO_TABLE_ROW_HEIGHT,
      payload: (SCREEN_HEIGHT - tableHeaderHeight * 2) / 10,
    });
  }, []);
  ...
  return (
    <NavigationContainer
      onStateChange={(state: any) =>
        setCurrentScreen(state.routes[state.routes.length - 1].name)
      }
      ref={navigationRef}
    >
      ...
      >
        ...
        >
          <Stack.Screen
            ...
            name={tabScreens.Paotable}
            component={PaotableScreen}
          />
          <Stack.Screen
            options={{
              headerShown: false,
            }}
            name={tabScreens.ProfileScreen}
            component={ProfileScreen}
          />
          <Stack.Screen
            options={{ headerShown: false }}
            name={tabScreens.WelcomeScreen}
